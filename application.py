...
def add_cash():
    if request.method == "POST":
        db.execute("UPDATE users SET cash = cash + :newCash WHERE id=:user_id", newCash = request.form.get("cash"), user_id=session["user_id"])
        return redirect("/")
    else:
...
def sell():
    if request.method == "POST":
        ...
        rows = db.execute("""
            SELECT symbol, SUM(shares) as totalShares
            FROM transactions WHERE user_id=:user_id
            GROUP BY symbol HAVING totalShares > 0;
            """, user_id=session["user_id"])
        for row in rows:
